  ...
  switch (this.schema.type) {
    ...
    case 'string':
      switch (this.schema.format) {
        ...
        case 'ObjectId':
          return parseObjectId(this.schema, value, propPath);
        default:
...
function parseObjectId(schema, value, propPath) {
  // Handle missing, required, and default
  value = getValueToValidate(schema, value);
  ...
  if (!mongoose.Types.ObjectId.isValid(value)) {
    throw ono({status: 400}, 'Cast to ObjectId failed for value "%s" at path "_id"', value);
  }
